  ...
  pdpnn <- function(zargs) {
    ...
    if (!is.null(pdp)) {
      ...
      if (is.factor(pdp[[vars[1]]])) vars <- rev(vars)
       ...
       p<-ggplot(data=pdp, aes(x=.data[[vars[1]]],y=fit,color=.data[[vars[2]]])) + geom_line()+
      ...
      p<-ggplot(data=pdp, aes(x=.data[[vars[1]]],y=.data[[vars[2]]])) + geom_tile(aes(fill=fit))+
